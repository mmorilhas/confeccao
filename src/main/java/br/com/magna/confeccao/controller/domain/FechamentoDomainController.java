...
public class FechamentoDomainController {
	...
	@GetMapping
	public ResponseEntity<Page<DadosListagemFechamentoDTO>> listar(@PageableDefault(size = 10, sort = { "id" }) Pageable paginacao) {
		return ResponseEntity.ok(repository.findAll(paginacao).map(DadosListagemFechamentoDTO::new)) ;
	}
